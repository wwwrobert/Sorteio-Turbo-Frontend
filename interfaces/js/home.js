...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const responseMessageTreino = document.getElementById("responseMessageAdicionar");
  const responseMessageCampeonato = document.getElementById("response-message-sortear");
  ...
  formTreino.addEventListener("submit", async function (e) {
    ...
    if (response.ok) { // Verifica se a resposta foi bem-sucedida (status 2xx)
      ...
      responseMessageTreino.innerHTML = "<p>Erro ao criar treino. Tente novamente.</p>";
    }
  });
  

  formCampeonato.addEventListener("submit", async function (e) {
    e.preventDefault();

    const nomeCampeonato = document.getElementById("nomeCampeonato").value;
    const quantidadeTimes = document.getElementById("quantidadeTimes").value;

    const response = await fetch(`${Url}/auth/campeonato`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    const data = await response.json();
    responseMessageCampeonato.innerHTML = `<p>${data.message}</p>`;

    setTimeout(() => {
      responseMessageCampeonato.innerHTML = "";
    }, 2000);
  });
});
